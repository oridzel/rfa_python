...
try:
    from trimesh.ray.ray_pyembree import (
        RayMeshIntersector as _EmbreeRayMeshIntersector,
    )
except Exception:
    # Embree/embreex is an optional acceleration dependency.
    _EmbreeRayMeshIntersector = None

from trimesh.ray.ray_triangle import (
    RayMeshIntersector as _TriangleRayMeshIntersector,
)
...
def _set_intersector_metadata(intersector, backend, embree_error=None):
    """
    Attach lightweight diagnostics without relying on them for collision logic.
    """
    try:
        intersector.rfa_backend = backend
        if embree_error is not None:
            intersector.rfa_embree_error = (
                f"{type(embree_error).__name__}: {embree_error}"
            )
    except Exception:
        # Metadata is diagnostic only; never block a valid intersector.
        pass

    return intersector


def _make_ray_intersector(collision_mesh: trimesh.Trimesh):
    """
    Build the fastest available Trimesh ray intersector.

    Embree/embreex is preferred for production trajectory tracking.  The
    triangle backend remains a transparent fallback when Embree is missing or
    cannot initialize in the current Python environment.
    """
    embree_error = None

    if _EmbreeRayMeshIntersector is not None:
        try:
            intersector = _EmbreeRayMeshIntersector(
                collision_mesh,
                scale_to_box=True,
            )
            ...
            return _set_intersector_metadata(
                intersector,
                backend="pyembree/embreex",
            )

    intersector = _TriangleRayMeshIntersector(collision_mesh)

    return _set_intersector_metadata(
        intersector,
        backend="triangle",
        embree_error=embree_error,
    )
...
def build_stl_intersector(meshes_for_collision: dict[str, trimesh.Trimesh]):
    """
    Build an Embree-first Trimesh intersector for a labeled collision mesh.

    The returned tuple is intentionally unchanged for compatibility with all
    existing callers: ``(collision_mesh, face_owner, intersector)``.
    """
    collision_mesh, face_owner = combine_labeled_meshes(meshes_for_collision)

    intersector = _make_ray_intersector(collision_mesh)
...
def is_in_drift_tube_aperture(
    ...
    """
    Check whether a spherical-shell point lies in the +X grid-frame opening.

    The supporting-frame opening around the drift tube is near +X and has
    radius r_hole in the YZ plane.

    r_hole defaults to field["grid_frame_opening_radius"] and falls back to
    the older field["drifttube_aperture_radius"] key, then to 5.6 mm.  This is
    deliberately not the tube's inner-bore radius: the actual aligned
    drift-tube STL decides whether a trajectory hits tube metal, while the
    annulus between the tube outer wall and the frame remains real vacuum.
    """
    ...
    if r_hole is None:
        ...
        r_hole = float(
            metadata.get(
                "grid_frame_opening_radius",
                metadata.get("drifttube_aperture_radius", 0.0056),
            )
        )
